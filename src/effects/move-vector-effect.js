/**
 * MoveVectorEffect - Animates a vector moving from one position to another
...
export class MoveVectorEffect extends TexToSVGShapeEffect {
  constructor(shape, originalStart, targetStart) {
    super(shape);
    this.originalStart = originalStart; // Original vector's start position
    this.targetStart = targetStart; // Target position to move to
    this.animationType = 'move';
    this.moveDuration = 0.8; // Duration for sliding animation in seconds
  }

  /**
   * Override toEndState to include moving to final position
   */
  toEndState() {
    ...
    // Then move it to final position instantly
    this.moveToFinalPosition();
  }

  /**
   * Instantly move the vector to the target position
   */
  moveToFinalPosition() {
  ...
  animateMove(onComplete) {
    ...
    TweenMax.to(animData, this.moveDuration, {
      progress: 1,
  ...
  /**
   * Override doPlay to show vector instantly then animate movement
   */
  doPlay(playContext) {
    try {
      ...
      // Then animate moving to target position
      this.animateMove(() => {
