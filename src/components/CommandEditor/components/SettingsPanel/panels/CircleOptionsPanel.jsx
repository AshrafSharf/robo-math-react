 ...
 */
const CircleOptionsPanel = ({ options, onChange }) => {
  const schema = getExpressionSchema('circle');
  const defaults = getDefaultOptions('circle');
  const currentOptions = { ...defaults, ...options };

  return (
    <div className="expression-options-panel circle-options">
        ...
        <div className="color-input-wrapper">
          ...
          <label className="option-checkbox-label inline">
            <input
              ...
              onChange={(e) => onChange('fill', e.target.checked ? '#3366CC' : 'none')}
            />
            <span>Enable</span>
          </label>
        </div>
...
export default CircleOptionsPanel;
