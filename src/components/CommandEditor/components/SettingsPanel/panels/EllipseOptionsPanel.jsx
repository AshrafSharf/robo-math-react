...
/**
 * Ellipse Options Panel
 * Options: strokeWidth, fill, fillOpacity
 */
const EllipseOptionsPanel = ({ options, onChange }) => {
  const schema = getExpressionSchema('ellipse');
  const defaults = getDefaultOptions('ellipse');
  const currentOptions = { ...defaults, ...options };

  return (
    <div className="expression-options-panel ellipse-options">
        ...
        <div className="color-input-wrapper">
          ...
          <label className="option-checkbox-label inline">
            <input
              ...
              onChange={(e) => onChange('fill', e.target.checked ? '#DC3912' : 'none')}
            />
            <span>Enable</span>
          </label>
        </div>
...
export default EllipseOptionsPanel;
