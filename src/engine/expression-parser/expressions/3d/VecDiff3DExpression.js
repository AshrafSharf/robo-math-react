/**
 * VecDiff3DExpression - creates the result vector of subtracting two 3D vectors
 *
 * Syntax:
 *   vecdiff3d(vecA, vecB)              - subtract vectors (a - b), result starts at origin
 *   vecdiff3d(vecA, vecB, point)       - subtract vectors, result starts at point
 *   vecdiff3d(vecA, vecB, x, y, z)     - subtract vectors, result starts at (x, y, z)
 *
 ...
 *   vecdiff3d(A, B)                        // result: (0,0,0) -> (2,1,1)
 *   vecdiff3d(A, B, point3d(g, 1, 1, 1))   // result: (1,1,1) -> (3,2,2)
 */
...
export class VecDiff3DExpression extends AbstractNonArithmeticExpression {
    static NAME = 'vecdiff3d';
    ...
    resolve(context) {
        if (this.subExpressions.length < 2) {
            this.dispatchError('vecdiff3d() requires: vecdiff3d(vecA, vecB)');
        }
        ...
        if (aType !== 'vector3d' && aType !== 'line3d') {
            this.dispatchError(`vecdiff3d() requires vector3d or line3d as first argument, got: ${aType}`);
        }
        ...
        const resultEnd = {
            x: resultStart.x + diffVec.x,
            y: resultStart.y + diffVec.y,
            z: resultStart.z + diffVec.z
        };
    ...
    getName() {
        return VecDiff3DExpression.NAME;
    }
    ...
    getFriendlyToStr() {
        ...
        return `vecdiff3d[(${pts[0].x.toFixed(2)}, ${pts[0].y.toFixed(2)}, ${pts[0].z.toFixed(2)}) -> (${pts[1].x.toFixed(2)}, ${pts[1].y.toFixed(2)}, ${pts[1].z.toFixed(2)})]`;
    }
