/**
 * PROVExpression - projects one vector onto another
 *
 * Syntax:
 *   prov(graph, vecToProject, vecTarget)  - project vecToProject onto vecTarget
 *
 * Returns the projection (parallel component) as a vector.
 *
 * Examples:
 *   A = vec(g, 0, 0, 3, 0)               // horizontal vector
 *   B = vec(g, 0, 0, 2, 2)               // diagonal vector
 *   prov(g, B, A)                        // projection of B onto A
 */
import { AbstractNonArithmeticExpression } from './AbstractNonArithmeticExpression.js';
import { VectorCommand } from '../../commands/VectorCommand.js';
import { VectorUtil } from '../../../geom/VectorUtil.js';

export class PROVExpression extends AbstractNonArithmeticExpression {
    static NAME = 'prov';
    ...
    resolve(context) {
        if (this.subExpressions.length < 3) {
            this.dispatchError('prov() requires: prov(graph, vectorToProject, vectorTarget)');
        }
        ...
        if (!this.graphExpression || this.graphExpression.getName() !== 'g2d') {
            this.dispatchError('prov() requires graph as first argument');
        }
    ...
    getName() { return PROVExpression.NAME; }
