/**
 * KatexTextItemMoveToCommand - Moves a KatexTextItem to a target position
 *
...
export class KatexTextItemMoveToCommand extends BaseCommand {
    ...
    async doInit() {
        ...
        if (this.graphExpression) {
            if (typeof this.graphExpression.getGrapher !== 'function') {
                console.warn('KatexTextItemMoveToCommand: graphExpression has no getGrapher method', this.graphExpression);
                return;
            }
            this.graphContainer = this.graphExpression.getGrapher();
            if (!this.graphContainer) {
                console.warn('KatexTextItemMoveToCommand: Could not get grapher from graphExpression');
                return;
            }
        }
        ...
        if (!textItemOrCollection) {
            console.warn(`KatexTextItemMoveToCommand: "${this.textItemVariableName}" not found in registry`);
            return;
        }
        ...
        if (!this.textItem) {
            console.warn('KatexTextItemMoveToCommand: No TextItem found');
            return;
        }
        ...
        if (!targetCoords) {
            console.warn('KatexTextItemMoveToCommand: Could not resolve target coordinates');
            return;
        }
    ...
    _resolveTargetCoordinates() {
        switch (this.targetType) {
            ...
            default:
                console.warn(`KatexTextItemMoveToCommand: Unknown target type "${this.targetType}"`);
                return null;
        }
    }
    ...
    _resolvePointTarget() {
        ...
        if (!this.graphContainer) {
            console.warn('KatexTextItemMoveToCommand: No graphContainer available');
            return null;
        }
        ...
        if (typeof this.graphContainer.toViewX !== 'function' || typeof this.graphContainer.toViewY !== 'function') {
            console.warn('KatexTextItemMoveToCommand: graphContainer missing toViewX/toViewY methods', this.graphContainer);
            return null;
        }
        ...
        if (pointExpression) {
            ...
            if (!pointShape) {
                console.warn(`KatexTextItemMoveToCommand: Point "${pointVariableName}" not found`);
                return null;
            }
        ...
        if (!modelPos) {
            console.warn('KatexTextItemMoveToCommand: Could not get point position');
            return null;
        }
    ...
    _resolveTextItemTarget() {
        ...
        if (!targetItemOrCollection) {
            console.warn(`KatexTextItemMoveToCommand: Target "${targetTextItemVariableName}" not found`);
            return null;
        }
        ...
        if (!targetItem) {
            console.warn('KatexTextItemMoveToCommand: No target TextItem found');
            return null;
        }
        ...
        if (!canvasBounds) {
            console.warn('KatexTextItemMoveToCommand: Could not get target TextItem bounds');
            return null;
        }
    ...
    _resolveLogicalTarget() {
        ...
        if (!layoutMapper) {
            console.warn('KatexTextItemMoveToCommand: No layoutMapper available');
            return null;
        }
