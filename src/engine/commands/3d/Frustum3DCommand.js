/**
 * Frustum3DCommand - Command for rendering a 3D frustum
 */
...
export class Frustum3DCommand extends Base3DCommand {
    /**
     * Create a frustum3d command
     * @param {Object} graphExpression - The graph expression
     * @param {Object} shapeData - { baseCenter: {x,y,z}, topCenter: {x,y,z}, baseRadius: number, topRadius: number }
    ...
    async doInit() {
        if (!this.graphExpression) {
            const err = new Error(common_error_messages.GRAPH_REQUIRED('frustum3d'));
        ...
        this.commandResult = this.graphContainer.diagram3d.frustum3d(
            this.shapeData.baseCenter,
            this.shapeData.topCenter,
            this.shapeData.baseRadius,
            this.shapeData.topRadius,
            '',
    ...
    async playSingle() {
        ...
        const effect = new Math3DShapeEffect(this.commandResult, 'frustum', { pen, camera, canvas });
        return effect.play();
    }
}
