/**
 * Prism3DCommand - Command for rendering a 3D prism
 */
...
export class Prism3DCommand extends Base3DCommand {
    /**
     * Create a prism3d command
     * @param {Object} graphExpression - The graph expression
     * @param {Object} shapeData - { baseCenter: {x,y,z}, sides: number, height: number, baseRadius: number }
    ...
    async doInit() {
        if (!this.graphExpression) {
            const err = new Error(common_error_messages.GRAPH_REQUIRED('prism3d'));
        ...
        this.commandResult = this.graphContainer.diagram3d.prism3d(
    ...
    async playSingle() {
        ...
        const effect = new Math3DShapeEffect(this.commandResult, 'prism', { pen, camera, canvas });
        return effect.play();
    }
}
