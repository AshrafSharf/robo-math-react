/**
 * ConeS3DCommand - Creates a cone in s3d space (pure Three.js)
 */
import { BaseCommand } from '../BaseCommand.js';

export class ConeS3DCommand extends BaseCommand {
    ...
    async doInit() {
        ...
        if (!diagram) {
            throw new Error('ConeS3DCommand: Could not find Space3DDiagram');
        }

        const parent = this._getParentObject();
        if (!parent) {
            throw new Error('ConeS3DCommand: Could not find parent Three.js object');
        }

        const mesh = diagram.cone(
            parent,
            this.center.x,
            this.center.y,
            this.center.z,
