...
export class CircleCommand extends BaseCommand {
    ...
    async doInit() {
        ...
        if (!this.graphExpression) {
            const err = new Error('Circle requires a graph container as first argument. Use: circle(g, x, y, radius)');
        ...
        if (!this.graphContainer) {
            const err = new Error('Graph container not initialized. Ensure g2d() is called before circle()');
            err.expressionId = this.expressionId;
            throw err;
        }
