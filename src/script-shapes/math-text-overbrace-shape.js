...
export class MathTextOverbraceShape {
  ...
  constructor(svgElement, bounds, options = {}) {
    ...
    this.bounds = bounds;
    this.id = `math-text-overbrace_${IdUtil.getID()}`;
  ...
  /**
   * Generate the overbrace path string
   * Creates a curly brace shape above the bounds
   */
  generatePath() {
    ...
    // Brace sits above the text
    const braceY = y - this.buffer;
  ...
  renderWithAnimation(penStartPoint, completionHandler) {
    try {
      ...
      console.error('MathTextOverbraceShape: Animation error', e);
      completionHandler();
    }
  }
