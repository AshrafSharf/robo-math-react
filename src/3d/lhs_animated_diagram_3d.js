/**
 * AnimatedDiagram class - extends Diagram with GSAP-based animations
...
import { animateLine } from './lhs/animator/lhs_line_animator.js';
import { fadeInPoint } from './lhs/animator/lhs_point_animator.js';
import { fadeInPolygon } from './lhs/animator/lhs_polygon_animator.js';
import { fadeInParallelogram } from './lhs/animator/lhs_parallelogram_animator.js';
import { fadeInLabel } from './lhs/animator/lhs_label_animator.js';
...
export class LHSAnimatedDiagram extends LHS3DDiagram {
    ...
    dashedVector(start, end, label = '', color = 0x00ff00, options = {}) {
        ...
        // Animate the dashed vector growth
    ...
    /**
     * Animated creation of a segment
     * @param {Object} start - Start point {x, y, z}
     * @param {Object} end - End point {x, y, z}
     * @param {string} label - Optional label
     * @param {number|string} color - Color
    ...
    rightAngleMarker(point1, vertex, point2, label = '', color = 0xff6600, options = {}) {
        ...
        // Create right angle marker with smaller default size
    ...
    planeByThreePoints(p1, p2, p3, label = '', color = 0x00ffff, options = {}) {
        // Calculate normal and delegate to planeByPointAndNormal
    ...
    /**
     * Animated creation of a parallelogram from two coterminal vectors
    ...
    dotProjection(vector, projectedOntoVector, label = '', color = 0xff0000, options = {}) {
        ...
        if (indicator) {
            ...
            // Get projection data
            const projectionData = indicator.userData.projectionData;
            const projectionDistance = projectionData.projectionDistance;
     ...
     */
    clear() {
        // Clean up trace animator
        if (this.traceAnimator) {
            this.traceAnimator.cleanup();
        }
    ...
    /**
     * Animated trace vector path - shows path animation for vector addition
     ...
     * @param {Object} options - Animation options
     * @returns {Promise} Promise that resolves when animation completes with cleanup function
     */
    traceVectorPath(directPath, vectorPairs, options = {}) {
        // Use the trace animator to show the animated path
        this.traceAnimator.animateTraceVectorPath(
            directPath, 
            vectorPairs,
            {
                duration: options.duration || this.animationDuration, // Use seconds directly
                traceColor: this.parseColor(options.traceColor || 'green'),
                directColor: this.parseColor(options.directColor || 'red')
            }
        );
    }
}