...
/**
 * Pure geometric utility functions that work in any coordinate system
...
/**
 * Translate a point along a direction by a given distance
 * @param {Object} point - Point to translate {x, y, z}
 * @param {Object} direction - Direction vector {x, y, z} (will be normalized)
 * @param {number} t - Distance to translate along the normalized direction
 * @returns {Object} Translated point {x, y, z}
 */
export function translatePoint(point, direction, t) {
    const normalizedDir = normalize(direction);
    return {
        x: point.x + normalizedDir.x * t,
        y: point.y + normalizedDir.y * t,
        z: point.z + normalizedDir.z * t
    };
}
...
/**
 * Finds the closest point on an infinite line to an external point
 * Uses projection to find the point on the line that minimizes distance
 * @param {Object} externalPoint - The external point {x, y, z}
 * @param {Object} pointOnLine - Any point on the line {x, y, z}
 * @param {Object} lineDirectionVector - Direction vector of the line {x, y, z}
 * @returns {Object} The closest point on the line {x, y, z}
 */
export function closestPointOnLine(externalPoint, pointOnLine, lineDirectionVector) {
    // Calculate vector from pointOnLine to externalPoint
    const PQ = subtractVectors(externalPoint, pointOnLine);
    
    // Project PQ onto lineDirectionVector to find parameter t
    // t represents how many times the direction vector we need to travel from pointOnLine
    const t = dotProduct(PQ, lineDirectionVector) / dotProduct(lineDirectionVector, lineDirectionVector);
    
    // Calculate the closest point on the line
    // closestPoint = pointOnLine + t * lineDirectionVector
    return {
        x: pointOnLine.x + lineDirectionVector.x * t,
        y: pointOnLine.y + lineDirectionVector.y * t,
        z: pointOnLine.z + lineDirectionVector.z * t
    };
}
