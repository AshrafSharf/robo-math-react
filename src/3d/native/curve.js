 ...
 * @param {Function} curveFunction - Function that takes parameter t and returns {x, y, z} in Three.js coordinates
...
export function parametricCurve(curveFunction, options = {}) {
    ...
    for (let i = 0; i <= segments; i++) {
        const t = tMin + i * step;
        const threePoint = curveFunction(t);
        
        // Skip invalid points
        if (!threePoint || 
            !isFinite(threePoint.x) || 
            !isFinite(threePoint.y) || 
            !isFinite(threePoint.z)) {
            continue;
        }
