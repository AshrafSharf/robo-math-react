/**
 * rhs_line_animator.js
 * GSAP animation methods for line objects using native Three.js coordinates
...
export function animateThinLine(lineObj, options = {}) {
    ...
    return TweenMax.to(animatedEnd, {
        x: endX,
        y: endY,
        z: endZ,
        duration: duration,
...
export function animateLine(cylinder, options = {}) {
    ...
    return TweenMax.to(cylinder.scale, {
        y: originalScaleY,
        duration: duration,
        ease: ease,
        onComplete: onComplete
 ...
 * @param {number} options.duration - Animation duration (default: 1)
 * @param {string} options.ease - GSAP easing function (default: "power2.inOut")
 * @param {boolean} options.animateDash - Whether to animate dash effect (default: true)
 * @param {Function} options.onComplete - Callback when animation completes
...
export function fadeInLine(lineObj, options = {}) {
    ...
    return TweenMax.to(lineObj.material, {
        opacity: originalOpacity,
        duration: duration,
        ease: ease,
        onComplete: onComplete
