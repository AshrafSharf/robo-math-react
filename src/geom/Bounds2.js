...
export class Bounds2 {
  ...
  maximumDistanceToPointSquared(point) {
    const x = point.x > this.getCenterX() ? this.minX : this.maxX;
    const y = point.y > this.getCenterY() ? this.minY : this.maxY;
    const dx = x - point.x;
    const dy = y - point.y;
    return dx * dx + dy * dy;
  }
   ...
   */
  toString() {
    return '[x:(' + Math.ceil(this.minX) + ',' + Math.ceil(this.maxX) + '),y:(' +  Math.ceil(this.minY) + ',' +  Math.ceil(this.maxY) + ')]';
  }
   ...
   * a rotation that isn't a multiple of π/2. This is because the returned bounds may expand in area to cover
   ...
   * a rotation that isn't a multiple of π/2. This is because the bounds may expand in area to cover
   * ALL of the corners of the transformed bounding box.
   *
  ...
  updateByPoints(topLeft, bottomRight) {
    this.minX = Math.min(topLeft.x, bottomRight.x);
    this.minY = Math.min(topLeft.y, bottomRight.y);
    this.maxX = Math.max(topLeft.x, bottomRight.x);
    this.maxY = Math.max(topLeft.y, bottomRight.y);
  }
  ...
  static byPoints(p1, p2) {
    return new Bounds2().withPoint(p1).withPoint(p2);
  }

  /**
   * A contant Bounds2 with minimums = ∞, maximums = -∞, so that it represents "no bounds whatsoever".
   * @public
   *
  ...
  /**
   * A contant Bounds2 with minimums = -∞, maximums = ∞, so that it represents "all bounds".
   * @public
   *
