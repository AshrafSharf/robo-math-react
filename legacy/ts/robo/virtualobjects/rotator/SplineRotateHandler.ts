...
module robo.virtualobjects.rotator
{
    ...
    export class SplineRotateHandler extends  RotatePartHandler
    {
        ...
        {
            var rotatedSpline:ProcessingSpline2D = <ProcessingSpline2D>itransformable.rotateTransform(angleInDegress,rotateAbout);
            ...
            var previewLinePart:GeometryPart = this.virtualRotator.rotatorGroup.spline(rotatedSpline.outPutAsPoint3D());
        ...
        public commitTransformableObject(itransformable:ITransformable,angleInDegress:number,rotateAbout:Point):void
        {
            ...
            this.commitSpline(itransformable,angleInDegress,rotateAbout);
        }

        private commitSpline(itransformable:ITransformable,angleInDegress:number,rotateAbout:Point):void
        {
            var rotatedSpline:ProcessingSpline2D = <ProcessingSpline2D>itransformable.rotateTransform(angleInDegress,rotateAbout);
            rotatedSpline =  <ProcessingSpline2D>rotatedSpline.translatePointForGraphSheetOffset(GraphSheet3D.translatePointForGraphSheetOffset)

            var outputMesh:Mesh = this.virtualRotator.ui3DScript.commitSpline(rotatedSpline.outPutAsPoint3D());
