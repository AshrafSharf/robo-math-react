...
module robo.virtualobjects.rotator
{
    ...
    export class LineRotatePartHandler extends  RotatePartHandler
    {
        ...
        constructor(virtualRotator:VirtualRotator)
        {
            ...
            this.lineOutputInstanceManager = new GeomPartInstanceRemoveManager(this.virtualRotator.rotatorGroup);
        }


        public drawPreviewObject(itransformable:ITransformable,angleInDegress:number,rotateAbout:Point):void
        {
            var rotatedLine:ProcessingLine2D = <ProcessingLine2D>itransformable.rotateTransform(angleInDegress,rotateAbout);
            ...
            var previewLinePart:GeometryPart = this.virtualRotator.rotatorGroup.line(startPt,endPt,this._lineThickness);
        ...
        public commitTransformableObject(itransformable:ITransformable,angleInDegress:number,rotateAbout:Point):void
        {
            ...
            this.commitLine(itransformable,angleInDegress,rotateAbout);
        }

        private commitLine(itransformable:ITransformable,angleInDegress:number,rotateAbout:Point):void
        {
            var rotatedLine:ProcessingLine2D = <ProcessingLine2D>itransformable.rotateTransform(angleInDegress,rotateAbout);
            rotatedLine =  <ProcessingLine2D>rotatedLine.translatePointForGraphSheetOffset(GraphSheet3D.translatePointForGraphSheetOffset);
            ...
            var startPt:Point3D = BaseVirtualObject.fromPointToPoint3D(rotatedLine.startPoint);
            var endPt:Point3D = BaseVirtualObject.fromPointToPoint3D(rotatedLine.endPoint);
            var outputMesh:Mesh = this.virtualRotator.ui3DScript.commitLine(startPt,endPt);
