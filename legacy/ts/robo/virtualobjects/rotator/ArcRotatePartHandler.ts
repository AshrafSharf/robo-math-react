...
module robo.virtualobjects.rotator
{
    ...
    export class ArcRotatePartHandler extends  RotatePartHandler
    {
        ...
        constructor(virtualRotator:VirtualRotator)
        {
            ...
            this.arcInstanceManager = new GeomPartInstanceRemoveManager(this.virtualRotator.rotatorGroup);
        }


        public drawPreviewObject(itransformable:ITransformable,angleInDegress:number,rotateAbout:Point):void
        {
            var processingCircle:ProcessingCircle = <ProcessingCircle>itransformable.rotateTransform(angleInDegress,rotateAbout);
            ...
            var previewEllipticArcPart:GeometryPart = this.virtualRotator.rotatorGroup.ellipticArc3d(origin,normal,axisVt,radius,radius,PMath.radians(fromAngleInDegrees),PMath.radians(toAngleInDegrees));
        ...
        public commitTransformableObject(itransformable:ITransformable,angleInDegress:number,rotateAbout:Point):void
        {
            ...
            this.commitArc(itransformable,angleInDegress,rotateAbout);
        }

        private commitArc(itransformable:ITransformable,angleInDegress:number,rotateAbout:Point):void
        {
            var processingCircle:ProcessingCircle = <ProcessingCircle>itransformable.rotateTransform(angleInDegress,rotateAbout);
            processingCircle =  <ProcessingCircle>processingCircle.translatePointForGraphSheetOffset(GraphSheet3D.translatePointForGraphSheetOffset);
            ...
            var outputMesh:Mesh = this.virtualRotator.ui3DScript.commitEllipticArc3d(origin,normal,axisVt,radius,radius,PMath.radians(fromAngleInDegrees),PMath.radians(toAngleInDegrees));
