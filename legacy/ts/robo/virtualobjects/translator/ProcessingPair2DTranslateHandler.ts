...
module robo.virtualobjects.translator {
    ...
    export class ProcessingPair2DTranslateHandler extends TranslatePartHandler {
        ...
        public drawPreviewObject(itransformable: ITransformable, transValue: Point, transAbout: Point): void {
            var processingPointPair2D: ProcessingPointPair2D = <ProcessingPointPair2D>itransformable.translateTransform(transValue, transAbout);
        ...
        public commitTransformableObject(itransformable: ITransformable, transValue: Point, transAbout: Point): void {
            ...
            this.commitSplineByPairPoints(itransformable, transValue, transAbout);
        }

        private commitSplineByPairPoints(itransformable: ITransformable, transValue: Point, transAbout: Point): void {
            var processingPointPair2D: ProcessingPointPair2D = <ProcessingPointPair2D>itransformable.translateTransform(transValue, transAbout);
            processingPointPair2D = <ProcessingPointPair2D>processingPointPair2D.translatePointForGraphSheetOffset(GraphSheet3D.translatePointForGraphSheetOffset)
