/**
 * Created by Mathdisk on 3/12/14.
 */

///<reference path="../../../libs/Away3D.next.d.ts" />
///<reference path="../_definitions.ts"/>

module robo.polyclipping
{
	export class ClipperBase
	{
		
		 static  horizontal:number = -3.4E+38;
		 static  loRange:number = 0x3FFFFFFF;          
		 static  hiRange:number = 0x3FFFFFFF;//FFFFFFFFL; Int64 not suppported 
		
		  m_MinimaList:LocalMinima;
		  m_CurrentLM:LocalMinima;
		  m_edges:TEdge[][] = [];
		  m_UseFullRange:Boolean;

		//------------------------------------------------------------------------------
		
		public static  abs(i:number):number
		{ 
			return i < 0 ? -i : i; 
		}
		//------------------------------------------------------------------------------

		public static  xor(lhs:Boolean, rhs:Boolean):Boolean 
		{ 
			return !( lhs && rhs ) && ( lhs || rhs ); 
		}
		//------------------------------------------------------------------------------
		
		public static  pointsEqual(pt1:IntPoint, pt2:IntPoint):Boolean
		{
			return pt1.equals(pt2);
		}
		//------------------------------------------------------------------------------

		  pointIsVertex(pt:IntPoint, pp:OutPt):Boolean
		{
			var pp2:OutPt = pp;
			do
			{
				if (ClipperBase.pointsEqual(pp2.pt, pt)) return true;
				pp2 = pp2.next;
			}
			while (pp2 != pp);
			return false;
		}
		//------------------------------------------------------------------------------

		  pointInPolygon(pt:IntPoint, pp:OutPt, useFulllongRange:Boolean):Boolean
		{
			var pp2:OutPt = pp;
			var result:Boolean = false;
			/*if (useFulllongRange)
			{
				do
				{
					if ((((pp2.pt.Y <= pt.Y) && (pt.Y < pp2.prev.pt.Y)) ||
						((pp2.prev.pt.Y <= pt.Y) && (pt.Y < pp2.pt.Y))) &&
					  new Int128(pt.X - pp2.pt.X) < 
					  Int128.Int128Mul(pp2.prev.pt.X - pp2.pt.X,  pt.Y - pp2.pt.Y) / 
					  new Int128(pp2.prev.pt.Y - pp2.pt.Y))
						result = !result;
					pp2 = pp2.next;
				} while (pp2 != pp);
			}
			else*/
			{
				do
				{
					if ((((pp2.pt.Y <= pt.Y) && (pt.Y < pp2.prev.pt.Y)) ||
						((pp2.prev.pt.Y <= pt.Y) && (pt.Y < pp2.pt.Y))) &&
						(pt.X - pp2.pt.X < (pp2.prev.pt.X - pp2.pt.X) * (pt.Y - pp2.pt.Y) /
						(pp2.prev.pt.Y - pp2.pt.Y))) result = !result;
					pp2 = pp2.next;
				} while (pp2 != pp);
			}
			return result;
		}
		//------------------------------------------------------------------------------

		  slopesEqual(e1:TEdge, e2:TEdge, useFullRange:Boolean):Boolean
		{
			/* if (useFullRange)
			{
				return Int128.Int128Mul(e1.ytop - e1.ybot, e2.xtop - e2.xbot) ==
					Int128.Int128Mul(e1.xtop - e1.xbot, e2.ytop - e2.ybot);
			}
			else */
			{
				return (e1.ytop - e1.ybot) * (e2.xtop - e2.xbot) -
					   (e1.xtop - e1.xbot) * (e2.ytop - e2.ybot) == 0;
			}
		}
		//------------------------------------------------------------------------------

		public  slopesEqual3(pt1:IntPoint, pt2:IntPoint, pt3:IntPoint, 
			useFullRange:Boolean):Boolean
		{
			/*if (useFullRange)
			{
				return Int128.Int128Mul(pt1.Y - pt2.Y, pt2.X - pt3.X) ==
				  Int128.Int128Mul(pt1.X - pt2.X, pt2.Y - pt3.Y);
			}
			else*/
			{
				return (pt1.Y - pt2.Y) * (pt2.X - pt3.X) - (pt1.X - pt2.X) * (pt2.Y - pt3.Y) == 0;
			}
		}
		//------------------------------------------------------------------------------

		public  slopesEqual4(pt1:IntPoint, pt2:IntPoint, pt3:IntPoint, pt4:IntPoint, 
			useFullRange:Boolean):Boolean
		{
			/*if (useFullRange)
			{
				return Int128.Int128Mul(pt1.Y - pt2.Y, pt3.X - pt4.X) ==
				  Int128.Int128Mul(pt1.X - pt2.X, pt3.Y - pt4.Y);
			}
			else */
			{
				return (pt1.Y - pt2.Y) * (pt3.X - pt4.X) - (pt1.X - pt2.X) * (pt3.Y - pt4.Y) == 0;
			}
		}
		//------------------------------------------------------------------------------

		public  ClipperBase()
		{
			this.m_MinimaList = null;
			this.m_CurrentLM = null;
			this.m_UseFullRange = false;
		}
		//------------------------------------------------------------------------------

		public  clear():void
		{
            this.disposeLocalMinimaList();
			for (var i:number = 0; i < this.m_edges.length; ++i)
			{
				for (var j:number  = 0; j < this.m_edges[i].length; ++j) this.m_edges[i][j] = null;
				this.m_edges[i].length = 0; //clear
			}
			this.m_edges.length = 0; // clear
			this.m_UseFullRange = false;
		}
		//------------------------------------------------------------------------------

		private  disposeLocalMinimaList():void
		{
			while( this.m_MinimaList != null )
			{
				var tmpLm:LocalMinima = this.m_MinimaList.next;
				this.m_MinimaList = null;
				this.m_MinimaList = tmpLm;
			}
			this.m_CurrentLM = null;
		}
		//------------------------------------------------------------------------------

		public  addPolygons(ppg:Polygons, polyType:number/*PolyType*/):Boolean
		{
			var result:Boolean = false;
            var polys:Polygon[]=ppg.getPolygons();

            for(var i:number=0;i<polys.length;i++)
            {
                if (this.addPolygon(polys[i],polyType))
                result = true;
            }


			return result;
		}
		//------------------------------------------------------------------------------


		public  addPolygon(polygon:Polygon, polyType:number/*PolyType*/):Boolean
		{
			var pg:IntPoint[] = polygon.getPoints();
			var len:number = pg.length;
			if (len < 3) return false;
			var newPoly:Polygon = new Polygon();
			var p:IntPoint[] = newPoly.getPoints();
			p.push(pg[0]);
			var j:number = 0;
			for (var i:number = 1; i < len; ++i)
			{

				var maxVal:number;
				if (this.m_UseFullRange) maxVal = ClipperBase.hiRange; else maxVal = ClipperBase.loRange;
				if (ClipperBase.abs(pg[i].X) > maxVal || ClipperBase.abs(pg[i].Y) > maxVal)
				{
					if (ClipperBase.abs(pg[i].X) > ClipperBase.hiRange || ClipperBase.abs(pg[i].Y) > ClipperBase.hiRange)
					{
						throw new ClipperException("Coordinate exceeds range bounds");
					}
					maxVal = ClipperBase.hiRange;
					this.m_UseFullRange = true;
				}

				if (ClipperBase.pointsEqual(p[j], pg[i]))
				{
					continue;
				}
				else if (j > 0 && this.slopesEqual3(p[j-1], p[j], pg[i], this.m_UseFullRange))
				{
					if (ClipperBase.pointsEqual(p[j-1], pg[i])) j--;
				} 
				else
				{
					j++;
				}
					
				if (j < p.length)
				{
					p[j] = pg[i]; 
				}
				else
				{
					p.push(pg[i]);
				}
			}
			if (j < 2) return false;

			len = j+1;
			while (len > 2)
			{
				//nb: test for point equality before testing slopes ...
				if (ClipperBase.pointsEqual(p[j], p[0])) j--;
				else if (ClipperBase.pointsEqual(p[0], p[1]) || this.slopesEqual3(p[j], p[0], p[1], this.m_UseFullRange))
					p[0] = p[j--];
				else if (this.slopesEqual3(p[j - 1], p[j], p[0], this.m_UseFullRange)) j--;
				else if (this.slopesEqual3(p[0], p[1], p[2], this.m_UseFullRange))
				{
					for (i = 2; i <= j; ++i) p[i - 1] = p[i];
					j--;
				}
				else break;
				len--;
			}
			if (len < 3) return false;

			//create a new edge array ...
			var edges:TEdge[] = [];
			for (i = 0; i < len; i++) edges[i] = new TEdge();
			this.m_edges.push(edges);

			//convert vertices to a double-linked-list of edges and initialize ...
			edges[0].xcurr = p[0].X;
			edges[0].ycurr = p[0].Y;
			this.initEdge(edges[len-1], edges[0], edges[len-2], p[len-1], polyType);
			for (i = len - 2; i > 0; --i)
			{
                this.initEdge(edges[i], edges[i + 1], edges[i - 1], p[i], polyType);
			}
            this.initEdge(edges[0], edges[1], edges[len-1], p[0], polyType);

			//reset xcurr & ycurr and find 'eHighest' (given the Y axis coordinates
			//increase downward so the 'highest' edge will have the smallest ytop) ...
			var e:TEdge = edges[0];
			var eHighest:TEdge = e;
			do
			{
				e.xcurr = e.xbot;
				e.ycurr = e.ybot;
				if (e.ytop < eHighest.ytop) eHighest = e;
				e = e.next;
			} while ( e != edges[0]);

			//make sure eHighest is positioned so the following loop works safely ...
			if (eHighest.windDelta > 0) eHighest = eHighest.next;
			if (eHighest.dx == ClipperBase.horizontal) eHighest = eHighest.next;

			//finally insert each local minima ...
			e = eHighest;
			do {
				e = this.addBoundsToLML(e);
			} while( e != eHighest );

			return true;
		}
		//------------------------------------------------------------------------------

		private  initEdge(e:TEdge, eNext:TEdge, ePrev:TEdge, pt:IntPoint, polyType:number):void
		{
			e.next = eNext;
			e.prev = ePrev;
			e.xcurr = pt.X;
			e.ycurr = pt.Y;
			if (e.ycurr >= e.next.ycurr)
			{
				e.xbot = e.xcurr;
				e.ybot = e.ycurr;
				e.xtop = e.next.xcurr;
				e.ytop = e.next.ycurr;
				e.windDelta = 1;
			} 
			else
			{
				e.xtop = e.xcurr;
				e.ytop = e.ycurr;
				e.xbot = e.next.xcurr;
				e.ybot = e.next.ycurr;
				e.windDelta = -1;
			}
            this.setDx(e);
			e.polyType = polyType;
			e.outIdx = -1;
		}
		//------------------------------------------------------------------------------

		private  setDx(e:TEdge):void
		{
			if (e.ybot == e.ytop) e.dx = ClipperBase.horizontal;
			else e.dx = (Number)(e.xtop - e.xbot)/(e.ytop - e.ybot);
		}
		//---------------------------------------------------------------------------

		private  addBoundsToLML(e:TEdge):TEdge
		{
			//Starting at the top of one bound we progress to the bottom where there's
			//a local minima. We then go to the top of the next bound. These two bounds
			//form the left and right (or right and left) bounds of the local minima.
			e.nextInLML = null;
			e = e.next;
			for (;;)
			{
				if ( e.dx == ClipperBase.horizontal )
				{
					//nb: proceed through ClipperBase.horizontals when approaching from their right,
					//    but break on ClipperBase.horizontal minima if approaching from their left.
					//    This ensures 'local minima' are always on the left of ClipperBase.horizontals.
					if (e.next.ytop < e.ytop && e.next.xbot > e.prev.xbot) break;
					if (e.xtop != e.prev.xbot) this.swapX(e);
					e.nextInLML = e.prev;
				}
				else if (e.ycurr == e.prev.ycurr) break;
				else e.nextInLML = e.prev;
				e = e.next;
			}

			//e and e.prev are now at a local minima ...
			var newLm:LocalMinima = new LocalMinima();
			newLm.next = null;
			newLm.Y = e.prev.ybot;

			if ( e.dx == ClipperBase.horizontal ) //ClipperBase.horizontal edges never start a left bound
			{
				if (e.xbot != e.prev.xbot) this.swapX(e);
				newLm.leftBound = e.prev;
				newLm.rightBound = e;
			} 
			else if (e.dx < e.prev.dx)
			{
				newLm.leftBound = e.prev;
				newLm.rightBound = e;
			} 
			else
			{
				newLm.leftBound = e;
				newLm.rightBound = e.prev;
			}
			newLm.leftBound.side = EdgeSide.LEFT;
			newLm.rightBound.side = EdgeSide.RIGHT;
            this.insertLocalMinima( newLm );

			for (;;)
			{
				if ( e.next.ytop == e.ytop && e.next.dx != ClipperBase.horizontal ) break;
				e.nextInLML = e.next;
				e = e.next;
				if ( e.dx == ClipperBase.horizontal && e.xbot != e.prev.xtop) this.swapX(e);
			}
			return e.next;
		}
		//------------------------------------------------------------------------------

		private  insertLocalMinima(newLm:LocalMinima):void
		{
			if( this.m_MinimaList == null )
			{
				this.m_MinimaList = newLm;
			}
			else if( newLm.Y >= this.m_MinimaList.Y )
			{
				newLm.next = this.m_MinimaList;
				this.m_MinimaList = newLm;
			} 
			else
			{
				var tmpLm:LocalMinima = this.m_MinimaList;
				while( tmpLm.next != null  && ( newLm.Y < tmpLm.next.Y ) )
					tmpLm = tmpLm.next;
				newLm.next = tmpLm.next;
				tmpLm.next = newLm;
			}
		}
		//------------------------------------------------------------------------------

		public  popLocalMinima():void
		{
			if (this.m_CurrentLM == null) return;
			this.m_CurrentLM = this.m_CurrentLM.next;
		}
		//------------------------------------------------------------------------------

		private  swapX(e:TEdge):void
		{
			//swap ClipperBase.horizontal edges' top and bottom x's so they follow the natural
			//progression of the bounds - ie so their xbots will align with the
			//adjoining lower edge. [Helpful in the ProcessClipperBase.horizontal() method.]
			e.xcurr = e.xtop;
			e.xtop = e.xbot;
			e.xbot = e.xcurr;
		}
		//------------------------------------------------------------------------------

		public  reset():void
		{
			this.m_CurrentLM = this.m_MinimaList;

			//reset all edges ...
			var lm:LocalMinima = this.m_MinimaList;
			while (lm != null)
			{
				var e:TEdge = lm.leftBound;
				while (e != null)
				{
					e.xcurr = e.xbot;
					e.ycurr = e.ybot;
					e.side = EdgeSide.LEFT;
					e.outIdx = -1;
					e = e.nextInLML;
				}
				e = lm.rightBound;
				while (e != null)
				{
					e.xcurr = e.xbot;
					e.ycurr = e.ybot;
					e.side = EdgeSide.RIGHT;
					e.outIdx = -1;
					e = e.nextInLML;
				}
				lm = lm.next;
			}
			return;
		}
		//------------------------------------------------------------------------------

		public  getBounds():IntRect 
		{
			var result:IntRect = new IntRect(0, 0, 0, 0);
			var lm:LocalMinima = this.m_MinimaList;
			if (lm == null) return result;
			result.left = lm.leftBound.xbot;
			result.top = lm.leftBound.ybot;
			result.right = lm.leftBound.xbot;
			result.bottom = lm.leftBound.ybot;
			while (lm != null)
			{
				if (lm.leftBound.ybot > result.bottom)
					result.bottom = lm.leftBound.ybot;
				var e:TEdge = lm.leftBound;
				for (; ; )
				{
					var bottomE:TEdge = e;
					while (e.nextInLML != null)
					{
						if (e.xbot < result.left) result.left = e.xbot;
						if (e.xbot > result.right) result.right = e.xbot;
						e = e.nextInLML;
					}
					if (e.xbot < result.left) result.left = e.xbot;
					if (e.xbot > result.right) result.right = e.xbot;
					if (e.xtop < result.left) result.left = e.xtop;
					if (e.xtop > result.right) result.right = e.xtop;
					if (e.ytop < result.top) result.top = e.ytop;

					if (bottomE == lm.leftBound) e = lm.rightBound;
					else break;
				}
				lm = lm.next;
			}
			return result;
		}
	}

}