 ...
 * @param {Object} point - Point on the surface in Three.js coordinates {x, y, z}
...
export function tangentPlane(point, partials, options = {}) {
    ...
    // Calculate normal vector from partial derivatives in Three.js space
    // For a surface z = f(x, y), the normal vector is (-∂f/∂x, -∂f/∂y, 1)
    const normalVec = new THREE.Vector3(-partials.dfdx, -partials.dfdy, 1);
    normalVec.normalize();
    
    // Create two orthogonal basis vectors for the plane in Three.js space
    // First basis vector: tangent in x-direction
    const basis1Vec = new THREE.Vector3(1, 0, partials.dfdx);
    basis1Vec.normalize();
    
    // Create second basis vector as cross product (perpendicular to both normal and basis1)
    const basis2Vec = new THREE.Vector3().crossVectors(normalVec, basis1Vec);
    basis2Vec.normalize();
    ...
    for (let i = 0; i <= segments; i++) {
        for (let j = 0; j <= segments; j++) {
            ...
            const vertex = threePoint.clone()
                .add(basis1Vec.clone().multiplyScalar(u))
                .add(basis2Vec.clone().multiplyScalar(v));
 ...
 * @param {Function} surfaceFunction - Function f(x, y) that returns z value in Three.js coordinates
 * @param {Object} point - Point on surface in Three.js coordinates {x, y} (z will be calculated)
 * @param {Object} options - Configuration options
 ...
 * @param {Object} point - Point on surface in Three.js coordinates {x, y, z}
