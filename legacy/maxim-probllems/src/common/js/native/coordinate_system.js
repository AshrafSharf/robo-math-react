...
export async function setupCoordinateSystem(scene, options = {}) {
    ...
    // Create or update OrthographicCamera
    let camera = scene.userData?.camera;
    const container = renderer.domElement.parentElement;
    const aspect = container.clientWidth / container.clientHeight;
    
    // If no camera exists, or if we have a PerspectiveCamera but need Orthographic for viewSize
    if (!camera || (camera.isPerspectiveCamera && viewSize !== undefined)) {
        // Store the current position if replacing an existing camera
        const oldPosition = camera ? camera.position.clone() : null;
        const oldTarget = camera && scene.userData?.controls ? 
            scene.userData.controls.target.clone() : null;
        ...
        // Restore position from old camera if it existed
        if (oldPosition) {
        ...
        // If we had controls, we need to recreate them with the new camera
        if (scene.userData?.controls) {
            scene.userData.controls.dispose();
            scene.userData.controls = null;
            // Controls will be recreated below if enableInteraction is true
        }
    } else if (camera.isOrthographicCamera) {
        // Update existing OrthographicCamera
