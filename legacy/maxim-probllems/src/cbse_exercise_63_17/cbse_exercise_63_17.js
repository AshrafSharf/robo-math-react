// Volume Maximization Problem - CBSE Exercise 63.17
...
const SHEET_SIZE = 18;  // Square sheet of 18 cm
const OPTIMAL_CUT = 3;  // Maximum volume occurs at x = 3 cm

// Store objectsMap for cleanup
let objectsMap = {};
let foldableNet = null;
let boxMesh = null;
let currentCutSize = OPTIMAL_CUT;  // Start at optimal value
...
function createLessonObjects(scene, cutSize) {
    ...
    // Sheet width measurement (18 units) - left side
    ...
    // Box base measurement (18 - 2x) - bottom side
    if (innerSize > 0.5) {
    ...
    // Add text labels
    // Label for cut size 'x' in corners
    ...
    // Label for base dimension '18 - 2x'
    if (innerSize > 2) {
        const baseLabel = label('18 - 2x',
            { x: -8, y: 0.5, z: 0 },
            {
                color: '#000000',
    ...
    if (innerSize > 0.5 && cutSize > 0.1) {
        ...
        boxMesh.position.set(8, cutSize/2, 0);
        ...
        const boxWidthLabel = label('18 - 2x',
...
function updateControlPanelDisplays(controlPanel, cutSize) {
    ...
    controlPanel.updateText('base-dimension',
        mathDisplay3D(
            `\\text{Base: } (18 - 2 \\cdot ${formatNumber(cutSize)})^2 = ${formatNumber(baseSize)}^2`,
            { block: true }
        )
    );
    ...
    controlPanel.updateText('height-dimension',
        mathDisplay3D(
            `\\text{Height: } x = ${formatNumber(cutSize)}`,
            { block: true }
        )
    );
    ...
    controlPanel.updateText('volume-value',
        mathDisplay3D(
            `V = ${formatNumber(volume, 1)} \\text{ cubic cm}`,
            { block: true }
        )
    );
    
    // Optimization section
    const dVdx = 12 * (cutSize * cutSize - 12 * cutSize + 27);  // From step-by-step.json
    
    controlPanel.updateText('derivative',
        mathDisplay3D(
            `\\frac{dV}{dx} = 12(x^2 - 12x + 27)\\\\= 12(${formatNumber(cutSize)}^2 - 12 \\cdot ${formatNumber(cutSize)} + 27) = ${formatNumber(dVdx, 1)}`,
            { block: true }
        )
    );
    ...
    controlPanel.updateText('critical-point',
        mathDisplay3D(
            `\\text{Critical points: } x = 3 \\text{ or } x = 9 \\text{ (only } x = 3 \\text{ is valid)}`,
            { block: true }
        )
    );
    ...
    if (Math.abs(cutSize - OPTIMAL_CUT) < 0.05) {
        controlPanel.updateText('max-indicator',
            mathDisplay3D(
                `\\text{Maximum: } V_{\\text{max}} = ${formatNumber(maxVolume, 1)} \\text{ cm}^3 \\text{ at } x = 3 \\quad \\color{green}{\\checkmark}`,
                { block: true }
            )
        );
    } else {
        controlPanel.updateText('max-indicator',
            mathDisplay3D(
                `\\text{Current: } ${percentOfMax}\\% \\text{ of max} \\quad (V_{\\text{max}} = ${formatNumber(maxVolume, 1)} \\text{ cm}^3 \\text{ at } x = 3)`,
                { block: true }
            )
        );
    }
...
export function render(scene, controlPanel) {
    ...
    controlPanel.createText('problem', 
        mathDisplay3D(`\\text{Square sheet of tin: } 18 \\text{ cm × } 18 \\text{ cm}`, { block: true }),
        tightStyle
    );
    
    controlPanel.createText('cut-description',
        mathDisplay3D(`\\text{Cut squares of side } x \\text{ from each corner}`, { block: true }),
        tightStyle
    );
