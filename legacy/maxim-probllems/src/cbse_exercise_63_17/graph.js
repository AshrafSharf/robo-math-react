// Graph module for plotting volume equation - CBSE Exercise 63.17
 ...
 * @param {number} options.sheetSize - Size of the square sheet (default: 18)
 * @param {number} options.currentCutSize - Current cut size to highlight (default: 3)
...
export function plot(scene, options = {}) {
    ...
    const graphHeight = 8;
    const xMin = 0;
    const xMax = sheetSize / 2;  // Maximum valid cut size is 9 for 18cm sheet
    const optimalCut = 3;  // From step-by-step.json
    const maxVolume = optimalCut * Math.pow(sheetSize - 2 * optimalCut, 2);  // 3 * 12^2 = 432
    const yMax = Math.ceil(maxVolume / 50) * 50;  // Round up to nearest 50 (450)
    ...
    // Add tick marks and labels on X-axis (0 to 9)
    ...
    // Add tick marks and labels on Y-axis (0 to 450)
    ...
    const optimalVolume = optimalCut * Math.pow(sheetSize - 2 * optimalCut, 2);
    const optimalMarkerX = xOffset - graphWidth/2 + optimalCut * xScale;
 ...
 * @param {number} sheetSize - Size of the sheet (default: 18)
 */
export function updateGraphMarker(graphGroup, newCutSize, sheetSize = 18) {
