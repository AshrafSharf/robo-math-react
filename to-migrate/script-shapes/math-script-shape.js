...
import { BaseStylableSubject } from '../ui-models/base-stylable-subject.js';
...
export class MathScriptShape extends BaseStylableSubject {
  constructor(modelCoordinates) {
    super();
    this.modelCoordinates = modelCoordinates;
    this.offsetUIValues = [0, 0];
    // Default style values directly embedded
    this.styleObj = {
      fill: 'transparent',
      'stroke-width': 2,
      'stroke': 'red',
  ...
  /**
   * Creates a MathShapeEffect for pen animation
   * @returns {MathShapeEffect} The effect that can be played
   */
  write() {
    console.log('Creating write effect for:', this.getShapeType());
    return new MathShapeEffect(this);
  }
  ...
  getViewCoordinates(modelCoordinates) {
    ...
    for (let i = 0; i < modelCoordinates.length; i += 2) {
      const x = this.graphsheet2d.toViewX(modelCoordinates[i]);
      const y = this.graphsheet2d.toViewY(modelCoordinates[i + 1]);
      coordinates.push(x);
      coordinates.push(y);
    }
    return coordinates;
   ...
   * @returns {MathScriptShape} Returns this for method chaining
   */
